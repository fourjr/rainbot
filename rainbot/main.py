...
from rainbot.services.database import DatabaseManager
...
class RainBot(commands.Bot):
    ...
    async def load_extensions(self) -> None:
        ...
        for file in os.listdir("rainbot/cogs"):
            if file.endswith(".py"):
                if self.dev_mode and file in ("logs.py",):
                    continue

                extension_name = f'rainbot.cogs.{file.replace(".py", "")}'
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if not message.author.bot and message.guild:
            # Optional dev-only guild gate for local testing (disabled)
    ...
    async def on_error(self, event_method: str, /, *args, **kwargs) -> None:  # type: ignore[override]
        ...
        try:
            ...
            # Truncate traceback to fit Discord's 1024 character limit for field values
            tb_truncated = tb if len(tb) <= 1024 else tb[-1024:]
            embed.add_field(
                name="Traceback",
                value=tb_truncated,
                inline=False,
    ...
    async def create_welcome_embed(self, guild: discord.Guild) -> discord.Embed:
        ...
        embed.add_field(
            name="🚀 Quick Start",
            value=f"1. Set up your prefix: `{config.DEFAULT_PREFIX}setprefix <prefix>`\n"
            f"2. Configure moderation: `{config.DEFAULT_PREFIX}setup`\n"
            f"3. Get help: `{config.DEFAULT_PREFIX}help`",
            inline=False,
        )
