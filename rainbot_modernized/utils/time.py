...
"""Adapted from https://github.com/Rapptz/RoboDanny/blob/rewrite/cogs/utils/time.py"""
...
class UserFriendlyTime(commands.Converter):
    ...
    async def check_constraints(
        self, ctx: commands.Context, now, remaining
    ) -> "UserFriendlyTime":
        if self.dt < now:
            raise commands.BadArgument("This time is in the past.")
        ...
        if not remaining:
            if self.default is None:
                raise commands.BadArgument(
                    "reason is a required parameter that is missing."
                )
